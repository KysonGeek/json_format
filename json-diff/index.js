...
window.vueApp = new Vue({
    ...
    mounted: function () {
        ...
        window.addEventListener('message', (e) => {
            if (!e || !e.data) return;
            if (e.data.type === 'json-diff-data') {
                const left = typeof e.data.left === 'string' ? e.data.left : JSON.stringify(e.data.left || {}, null, 4);
                const right = typeof e.data.right === 'string' ? e.data.right : JSON.stringify(e.data.right || {}, null, 4);
                jsonBox.left.setValue(left || '');
                jsonBox.right.setValue(right || '');
                jsonBox.left.refresh();
                jsonBox.right.refresh();
                setTimeout(() => this.compareJson(), 100);
            }
        });
